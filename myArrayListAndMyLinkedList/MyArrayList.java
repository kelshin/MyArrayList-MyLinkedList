...
public class MyArrayList<E> implements List<E> {
    ...
    public boolean addAll(int index, Collection<? extends E> c) {
        // TODO: Exercise
        Object[] a = c.toArray();
        int mainLength = elementData.length;
        int length = a.length;
        if (length == 0) {
            return false;
        }
        ...
        Object[] reserves = new Object[size - index];
        System.arraycopy(elementData, index, reserves, 0, reserves.length);
        System.arraycopy(a, 0, elementData, index, length);
        System.arraycopy(reserves, 0, elementData, length + index, reserves.length);
        size += length;
    ...
    public boolean retainAll(Collection<?> c) {
        // TODO: Exercise\
        Object[] check = c.toArray();
        for (int i = 0; i < check.length; i++){
            int loopSize = size - 1;
            while (loopSize >= 0){
                if (check[i].equals(elementData[loopSize])){
                    fastRemove(loopSize);
                }
                loopSize--;
            }
